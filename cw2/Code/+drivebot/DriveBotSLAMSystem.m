...
%                 'Implement the rest of this method for Q1b.');
        ...
        function deleteVehiclePredictionEdges(this)
            ...
            % Number of edges
            numbersOfEdges = length(allEdges);

            currentEdgeIndex = 0;
            % For edge in list of edges
            for e = 1:numbersOfEdges
                % Get the current edge
                edge = allEdges{e};
                % If the edge is vehicle kinematics edge
                if class(edge) == "drivebot.graph.VehicleKinematicsEdge"
                    currentEdgeIndex = currentEdgeIndex + 1;
                    % Skip removing the first edge if keepFirstPredictionEdge is true
                    if (currentEdgeIndex == 1 && this.keepFirstPredictionEdge == true)
                        disp("Skipping first edge");
                    else
                        % Remove the edges
