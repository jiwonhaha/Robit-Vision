...
classdef DriveBotSLAMSystem < minislam.slam.SLAMSystem
    ...
    methods(Access = public)
        ...
        function setGraphPruning(this, toPruneOrNot)
            this.useGraphPrune = toPruneOrNot;
        end 
    end
    ...
    methods(Access = protected)
        ...
        function handlePredictToTime(this, time, dT)
            ...
            % Create and add the process model edge
            ...
            X_pre = this.vehicleVertices{k}.estimate();

            % Predict the state using odometry and noise
            M = [cos(X_pre(3)) -sin(X_pre(3)) 0;
                 sin(X_pre(3))  cos(X_pre(3)) 0;
                 0              0             1];
            
            % Predict the post state
            X_post = X_pre;
            X_post = X_post + (dT * M * odometry);
            X_post(3) = g2o.stuff.normalize_theta(X_post(3));  
            this.currentVehicleVertex.setEstimate(X_post);
...
%                 'Implement the rest of this method for Q1b.');
        ...
        function handleGPSObservationEvent(this, event)
            ...
            e = drivebot.graph.GPSMeasurementEdge(event.xyOffset);
        ...
        function handleLandmarkObservationEvent(this, event)
            ...
            for l = 1 : length(event.landmarkIds)
                ...
                % Q2b:
                % Create a new LandmarkRangeBearingEdge
                landmarkRangeBearingEdge = drivebot.graph.LandmarkRangeBearingEdge();
                
                % Set the vertices (current vehicle state and landmark state) to the edge
                landmarkRangeBearingEdge.setVertex(1, this.currentVehicleVertex);
                landmarkRangeBearingEdge.setVertex(2, landmarkVertex);
                
                % Set the observation (measurement) and information (inverse of covariance) matrix to the edge
                landmarkRangeBearingEdge.setMeasurement(z);
                
                % Compute the 2x2 information matrix by taking the inverse of the covariance matrix
                omega = inv(event.covariance);

                landmarkRangeBearingEdge.setInformation(omega);
        ...
        function deleteVehiclePredictionEdges(this)
            ...
            % Number of edges
            numberOfEdges = length(allEdges);

            vehicleKinematicsEdgeIndex = 0;

            % Iterate over each edge in the graph.
            for idx = 1:numberOfEdges
                % % Get the current edge.
                edge = allEdges{idx};
                
                % Check if the current edge is a vehicle kinematics edge.
                if isa(edge, 'drivebot.graph.VehicleKinematicsEdge')
                    vehicleKinematicsEdgeIndex = vehicleKinematicsEdgeIndex + 1;
                    
                    % Determine if the first prediction edge should be skipped.
                    if vehicleKinematicsEdgeIndex == 1 && this.keepFirstPredictionEdge
                        disp('Skipping first vehicle kinematics edge.');
                    else
        ...
        % Q3b:
        % Calculate the density of each vertex
        % Remove edges from the vertices that have density more than
        % threshold
        function graphPruning(this)
            ...
            % Define maximum density threshold
            densityThreshold = 100;

            % Collect the number of removed edges and associated vertices
            numberOfRemovedEdges = 0;
            numberOfReducedVertices = 0;

            % Average density
            d_avg = 0;
    
            % Calculate density of every vertex
            for i = 1:length(allVertices)
                % If the current vertex is a vehicle state vertex
                if (isa(allVertices{i}, 'drivebot.graph.VehicleStateVertex'))
                    % Get the position of the current vertex
                    current_vertex = allVertices{i}.estimate();
    
                    % Density
                    d_i = 0;
                    % Cummulate the density
                    for j = 1:length(allVertices)
                        % If the correspondence vertex is a vehicle state vertex
                        if isa(allVertices{j}, 'drivebot.graph.VehicleStateVertex') && i ~= j
                            % Get the position of the correspondance vertex
                            distant_vertex = allVertices{j}.estimate();
                            % Distance between current vertex and
                            % correspondance vertex
                            r_i = sqrt((current_vertex(1) - distant_vertex(1))^2 + (current_vertex(2) - distant_vertex(2))^2);
                            d_i = d_i + 1 / r_i;
                        end
                    end
                    d_i = d_i * 1/pi;
                    ...
                    % If the density is more than threshold
                    if d_i > densityThreshold
                        % Get the list of attaching edges
                        edgesList = edges(allVertices{i});
                        numberOfReducedVertices = numberOfReducedVertices + 1;
                        % For edge in list of attaching edges
                        for e = 1:length(edgesList)
                            % If the edge is vehicle kinematics edge
                            if class(edgesList{e}) == "drivebot.graph.VehicleKinematicsEdge" && edgesList{e} ~= allEdges{1}
                                % Remove the edge
                                this.graph.removeEdge(edgesList{e});
                                numberOfRemovedEdges = numberOfRemovedEdges + 1;
                            end
                        end
                    end
                end
            end
            fprintf('Removing %d edges from %d vertices!\n', numberOfRemovedEdges, numberOfReducedVertices);
            fprintf('Average density: %f\n', d_avg / length(allVertices));
        end
