    ...
    methods(Access = public)
        ...
        function linearizeOplus(this)
            ...
            %warning('landmarkrangebearingedge:linearizeoplus:unimplemented', ...
                %'Implement the rest of this method for Q1b.');
            vehicleEstimate = this.edgeVertices{1}.estimate(); 
            landmarkEstimate = this.edgeVertices{2}.estimate();
            distanceVector = landmarkEstimate - vehicleEstimate(1:2); 
            range = norm(distanceVector); 

            % Jacobian matrices for vehicle and landmark vertices
            this.J{1} = ...
                [-distanceVector(1)/range, -distanceVector(2)/range, 0;
                 distanceVector(2)/(range^2), -distanceVector(1)/(range^2), -1];
